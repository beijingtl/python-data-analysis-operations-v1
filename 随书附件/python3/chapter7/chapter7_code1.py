...
for dimension in dimension_list:  # 遍历每个维度
    ...
    the_day_groupby = pd.DataFrame(the_data_merge.iloc[:, -1].groupby(the_data_merge.iloc[:, 0]).sum())  # 对指定日期特定维度汇总求和
    previous_day_groupby = pd.DataFrame(
        previous_data_merge.iloc[:, -1].groupby(previous_data_merge.iloc[:, 0]).sum())  # 对指定日期前1天特定维度汇总求和
    # part2
