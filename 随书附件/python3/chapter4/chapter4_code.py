...
for each_data in graph_data.values:  # 循环读出每条规则
    ...
    node1_str = ''.join(node1)  # 转化为字符串
    node2_str = ''.join(node2)  # 转化为字符串
    label = '%s' % weight  # 创建一个标签用于展示实例数
    dot.node(node1_str, node1_str, shape='record')  # 增加节点（规则中的前项）
    dot.edge(node1_str, node2_str, label=label, constraint='true')  # 增加有向边
